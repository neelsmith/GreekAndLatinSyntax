...
md"""*Notebook version:* **0.1.0**  *See version history* $(@bind history CheckBox())
"""
...
- **0.1.0** fixes all bugs recorded in 0.0.1, and adds:
    - display of morphology codes on hover
...
begin
	tbdmd = md"""### Goals for `0.1.0`


- ✅ add HTML hover with morphology info
- export "[in usum](https://www.inusum.com)"-style reader
- ✅ fix documented bugs in `0.0.1`
"""
	warning_box(tbdmd)
end
...
# ╔═╡ d6d63d72-71b4-42d7-abbe-93f8f297f061
md"""*Manually add title and id for text we're annotating until full CITE-compliant source is available*:"""

# ╔═╡ ccb5e655-9bb1-4117-9231-f9a90855b48b
title = "Lysias 1"

# ╔═╡ 82dafeff-8f2d-4116-96c5-029f2f9dac98
id = "placeholder_for_urn"
...
if openhood
	md"""> ### Prerequisites documentation: loading and formatting data
>
...
if openhood
	md"""> ### Step 1 documentation: annotating a sentence
...
if openhood
md"""> ### Step 2 documentation. Define verbal units
>
...
if openhood
md"""> ### Step 4 documentation. Annotate syntax
>
...
if openhood
md"""> ### Documentation: file management
>
> 
>
...
# ╔═╡ 99b16f15-0fdf-46a9-be4a-8b6d130fbb38
htmldir = mkpath(joinpath(pwd(), "output", "html"))

# ╔═╡ 23546c93-15f4-435b-a2a3-5590f5e3c918
cexdir = mkpath(joinpath(pwd(), "output", "delimited"))

# ╔═╡ 03a20740-0756-4081-b2f6-335888238336
if openhood
md"""> ### Documentation: other UI functions
>
...
function formatsentenceBW(s)
	...
	for (i, tkn) in enumerate(s)
		morphcode =  tkn[3] 
		if morphcode == "u--------" # punctuation
			push!(formatted, "$(tkn[2])")
			
		elseif ismissing(connectorid) || isnothing(connectorid) || ! (i == connectorid)
			push!(formatted, " <span class=\"tooltip\"  tool-tips=\"$(morphlabel(morphcode))\")>$(tkn[2])</span>")
		else
			tagged = "<span class=\"tooltip connector\"  tool-tips=\"$(morphlabel(morphcode))\")>$(tkn[2])</span>"
...
function coloredformat()
	...
	if isnothing(assignedtokensdf)
		...
		for r in eachrow(assignedtokensdf)
				#class=\"tooltip\" tool-tips=\"$(morphlabel(morphcode))\")
			 groupidx = groupidx + 1
			morphcode = sentence[groupidx][3]
			if groupidx == connectorid
					push!(groupedtext, " <span class=\"tooltip connector\" tool-tips=\"$(morphlabel(morphcode))\">$(r.token)</span>")
				...
				elseif r.group == 0
					push!(groupedtext, " <span class=\"tooltip unassigned\" tool-tips=\"$(morphlabel(morphcode))\">$(r.token)</span>")
				else
					coloridx = mod(r.group, length(palette)) + 1
					push!(groupedtext, " <span class=\"tooltip\" style=\"color: $(palette[coloridx])\" tool-tips=\"$(morphlabel(morphcode))\">$(r.token)</span>")
...
if step3() 
	HTML("""<p>Define syntactic relations for:<br/><blockquote>
$(coloredformat())</blockquote></p>
	""")
end
...
if step1()
	...
	HTML("<i>Define verbal units for:</i><br/><br/><blockquote><strong>Sentence $(sentid)</strong>: " * str * "</blockquote>")
...
function displaysentence()
	...
	elseif isnothing(connectorid)
			str = formatsentence(sentences[sentid])
			HTML("<p><blockquote><span class=\"connector tt\">Asyndeton: no connecting word.</span><br/><strong>Sentence $(sentid)</strong>: " * str * "</blockquote></p>")
		
	else
		
		str = formatsentence(sentences[sentid])
		HTML("<p><blockquote><strong>Sentence $(sentid)</strong>: " * str * "</blockquote></o>")
	end
end;
