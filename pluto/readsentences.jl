...
if history
	md"""
- **0.2.2**: use updated `GreekSyntax` package; add options to use default or customized CSS and color palette
...
begin
	if seecss
	cssmsg = md"""
The following cells define the visual appearance of the text's formatting.  If you are familiar with CSS, you can modify them to tailor the presentation to your preferences.

1.  Unfold the `Summary of CSS usage` to see what attributes are defined in the HTML generated by the `GreekSyntax` package.  You can alter the corresponding CSS values in the definition of `css` in the following cell.  You can also check the *Use default CSS* box to use the default CSS definitions in the `GreekSyntax` package.
2. The second cell defines `palette`, a series of colors that the notebook cycles through in highlighting different verbal expressions by color.  You can directly edit the cell to change those RGB values.  You can also check the *Use default color palette* box to use the default definitions in the `GreekSyntax` package.
"""
